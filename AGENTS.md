...
- Batch project assignment and grouped description edits must use Toggl Track bulk update operations instead of one request per entry.
- Bulk updates are limited to 100 time entries per request; split larger selections into chunks.
- Preserve partial-failure handling: update local state only for successful entry IDs and report failed IDs/messages to the user.
- Track `X-Toggl-Quota-Remaining` and `X-Toggl-Quota-Resets-In` headers from API responses.
- Treat `429 Too Many Requests` as a retryable rate-limit response.
- Treat `402 Payment Required` as quota exhaustion and wait for the reset window when available.
- Keep TUI footer/status messaging aligned with the latest known quota state so users understand when batch operations are delayed or blocked.
